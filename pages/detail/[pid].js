...
const Detail = () => {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    query.equalTo('objectId', pid).find().then(async (response) => {
      ...
      if (mapped.length) {
        ...
        const master = polygonNFTs?.result?.find(m => m?.token_id === resNft.token_id)
...
console.log(resNft);
  ...
  const getCollection = async (tokenAddress) => {
    ...
    return await query.equalTo('token_address', tokenAddress).find().then(async response => {
                      ...
                      <div className="card-body">
                        <div className="prod-slider-main">
                          <div className="prod-slider-head">
                            <h4>{nft?.name}</h4>
                            <h6><span>{nft?.available} of {nft?.quantity} left</span></h6>
                          </div>
                          ...
                          <img
                            src={nft?.image_uri}
                            className={'rounded mx-auto d-block rounded img-wrap mt-5'}
                            width={'100%'}
                          />
                  ...
                  <div className="col-xxl-4 col-xl-12">
                    <div className="card welcome-Detail">
                      <div className="card-body prod-price-wid">
                        <h4><span>{nft?.price} MARTEX</span></h4>
                        <span className="qty">{nft?.available} of {nft?.quantity} left</span>
                        {buyButtons()}
                      </div>
                      <div className="card">
