...
import {useMoralisQuery, useNewMoralisObject, useMoralisWeb3Api, useMoralis} from 'react-moralis';
            ...
            try {
                await Web3Api.token.getAllTokenIds(options).then(async response => {
                    ...
                    const mapped = nfts.map(nft => {
                        ...
                        const master = masterNftsRes.find(m => m.token_id === nft.token_id)
                    ...
                    }).filter(el => el?.sale && el?.featured).sort((a,b) => (b.token_id-a.token_id));
                            ...
                            <div className="card-body bs-0 p-0 bg-transparent">
                                {isLoading ? (
                                  ...
                                  <div className="row">
                                      ...
                                      {nfts.map((nft, index) => (<NFTCard item={nft} collections={collections} fetchNFTs={initial} key={index}/>))}
                                  </div>
                                )}
                            </div>
                        </div>
